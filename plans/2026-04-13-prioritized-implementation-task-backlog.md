...
Cross-check against `plans/2026-04-13-repo-state-gap-analysis.md` **v1.7** and `plans/2026-04-13-claims-verification-baseline.md` **v1.4** ( **`plans/2026-04-18-streetview-google-perpendicular-sampling-full-scope.md`** v0.3 = stochastic DoD; road-bearing deferred). **Publishable UI / store-ready shell:** dedicated band **IMP-140–IMP-149** — see [`plans/2026-04-21-publishable-ui-stitch-parity-and-ship-criteria.md`](2026-04-21-publishable-ui-stitch-parity-and-ship-criteria.md) (converged SCAN hub, theme layer, checklist completion, **permissive** manifest UX + local-first sync, BGM, release checklist; **IMP-148 visual regression deferred**); exit gates in [`docs/PUBLISHABLE-UI-EXIT-CRITERIA.md`](../docs/PUBLISHABLE-UI-EXIT-CRITERIA.md).
...
| **IMP-020** | **`rootProject.name = "nutonic"`**, **`com.nutonic.*`** — **landed**; retired KMP photo gallery sample **removed** (**gap analysis v1.7**). |
...
| **IMP-072** | **Client:** **manifest-first** SCAN catalog via **`refreshScanHubCatalog`** (`com.nutonic.shell.ScanHubCatalog`) when **`ContentCacheRepository.refreshManifest()`** yields map rows; else **`GET /api/v1/maps`**. **Server:** static **`PUBLISHED_MAPS`** (DB-backed index still future). **Verdict:** client **S1c catalog path** **landed** alongside **IMP-080**; normative “full **S1c**” still allows **DB-backed** catalog later. |
...
| **IMP-081** | **Landed (shipped bytes):** **`git`** tracks **`composeResources/files/maps/*.jpg`**, **`manifest.full.json`** still paths, and **`server/.../bundles/*.jpg`** + **`registry.json`** (multi-map **`nutonic.still.v1.*`** plus demo rows); **`:shared:validateCatalog`** / **`validate_shipped_compose_resources.py`** passes on a normal clone. **Remaining:** **operational** — re-run **`data/scripts/render_mapbox_still.py`** / **`assemble_manifest`** / **`sync_server_catalog.py --write`** when **`data/catalog/`** or still policy changes (**`plans/2026-04-14-shipped-cache-narrative-hint-pipeline.md`** §5 **B**/**F**). |
...
| **IMP-084** | **Partial:** final results surface + local summary + RANK navigation; **`shareNutonicScorecard`** is implemented on Android / Desktop / iOS / Web (`navigator.share` + clipboard fallback on Web). **Open:** normalize **share overlay lifecycle** (loading/success/error + non-blocking navigation) and unify behavior across platforms. |
| **IMP-060** | **Landed** as before. **Optional:** refresh-token table — **not** implemented. |
| **IMP-060+** | **Landed (2026-04-16 verify):** **IMP-090** ranked **start/submit/forfeit** + SQLite store; **IMP-092** **`InferenceClient`** for **PRO** health probes + HMAC + optional materialize forward. **Still open:** **IMP-092** circuit breaker / full multi-worker **`TOPOLOGY`**; **IMP-110** plan **§13.4** / **§14** (remaining observability, docs, optional checklist — chunked LFM **`rank`** merge + **`reports/model_pins.json`** **landed** in **`tools/batch_streetview_hints.py`**; **not** road-bearing per **[`plans/2026-04-18-streetview-google-perpendicular-sampling-full-scope.md`](2026-04-18-streetview-google-perpendicular-sampling-full-scope.md)** v0.3); full **`inference/*`** non-stub deploy defaults per **`plans/2026-04-16-stub-replacement-implementation-plan.md`**. **Closed vs earlier text:** “manifest depth” is **no longer** a blanket blocker — server + client manifest **landed**; **git-shipped** still JPEGs + server **`bundles/`** mirror **landed** (gap analysis **v1.6**); **ranked clue UX** polish remains. |
| **IMP-140–149** | **In progress (band active):** publishable UI / vendored parity / release readiness per **`plans/2026-04-21-publishable-ui-stitch-parity-and-ship-criteria.md`**. **Current status:** SCAN/SETUP/results/checklist surfaces and permissive offline-first behavior are partially landed; remaining work is release-grade QA pass, copy tightening across all routes, and final sign-off artifacts. **IMP-148 (visual regression CI) remains deferred by plan AD-5.** |
...
| **IMP-081** | Ship **one canned bundle** (Mapbox still + metadata) for dev — no HF Jobs required. | **2026-04-21 verify:** **Landed** for **shipped** multi-map stills — **`composeResources/files/maps/*.jpg`**, **`manifest.full.json`**, **`server/.../bundles/*.jpg`** + **`registry.json`** are **git-tracked**; **`GET /api/v1/bundles/...`** serves bytes. **Remaining:** document / automate **regeneration** when catalog or still policy changes (**shipped-cache plan** §5 **B**/**F**). | IMP-080 | Client renders still + map; round loads offline-capable. | `docs/GAME-ENGINE.md` §9; **shipped-cache plan** §5 |
...
## Wave W11 — Publishable UI / vendored parity / release readiness

Primary source of truth: **`plans/2026-04-21-publishable-ui-stitch-parity-and-ship-criteria.md`** (phases P0–P8), with release gates in **`docs/PUBLISHABLE-UI-EXIT-CRITERIA.md`** and rule constraints in **`rules/15-publishable-ui-and-release-readiness.md`**.

| ID | Task | Subtasks | Deps | Acceptance | Refs |
|----|------|----------|------|------------|------|
| **IMP-140** | Vendored UI contract pack for parity review (no stitched asset wiring). | 140.1 Keep `refs/stitch/` optional/local-only (gitignored). 140.2 Promote required structure/copy/layout contracts into first-party docs (`docs/VENDORED-UI-CONTRACT-PACK.md`, `rules/07`, `docs/NU_TONIC_ARTIFACT_REFERENCE.md`, publishable plan). 140.3 Document contributor flow without submodule/init dependency. | IMP-000 | Publishable parity and release sign-off do not depend on checking in `refs/stitch/`; review uses vendored contracts. | `plans/2026-04-21-publishable-ui-stitch-parity-and-ship-criteria.md` §P0; `docs/VENDORED-UI-CONTRACT-PACK.md`; `rules/15` |
| **IMP-141** | Converged SCAN hub (replace stub-only SCAN IA). | 141.1 Replace SCAN `NavStubButton` quartet with one route-level composable containing mission + map/level + leaderboard slice + play actions. 141.2 Keep shared `mapContextId` + `rankFocusMapId` contract from **IMP-071**. 141.3 Deprecate standalone Mission/Map placeholders for release builds (debug-only deep links allowed). | IMP-071, IMP-072 | `rules/07` #4b/#4c must-include items exist in SCAN without navigating placeholder-only routes. | `rules/01`; `rules/07`; `docs/GAME-ENGINE.md`; publishable plan §P1 |
| **IMP-142** | Publishable Compose component layer and token discipline. | 142.1 Add `NutonicGlassCard`, `NutonicPrimaryButton`, `NutonicGhostButton`. 142.2 Extend theme tokens (`surfaceGlass`, `outlineGhost`, CTA 12–16dp, BGM crossfade param). 142.3 Replace remaining Material-default chrome on shipped surfaces. | IMP-040, IMP-051 | No raw design hex in feature screens; SCAN/gameplay/readability aligns with `docs/DESIGN.md` + `rules/02`. | `docs/DESIGN.md`; `rules/02`; publishable plan §2.2/§P2 |
| **IMP-143** | Checklist screen completion (shell foundations). | Implement non-placeholder Splash/Auth/Role/Success/Final-results surfaces with release-safe copy and route-correct BGM tracks. | IMP-141, IMP-142 | `rules/07` rows #1/#2/#3/#6/#7 pass manual checklist and release copy audit (no `IMP-`/`rules/` strings). | `rules/07`; `docs/SCREEN-MUSIC-SPEC.md`; publishable plan §P3 |
| **IMP-144** | Checklist screen completion (INTEL/SETUP/PRO depth). | 144.1 INTEL minimum layout (`docs/INTEL-TAB-SPEC.md` §1). 144.2 SETUP grouped sections from `docs/CLIENT-SETTINGS-SPEC.md` §6. 144.3 Functional PRO strip (not title-only placeholder). | IMP-143 | INTEL/SETUP/PRO are not placeholder-only; settings persist + reflect in UI. | `docs/INTEL-TAB-SPEC.md`; `docs/CLIENT-SETTINGS-SPEC.md`; `docs/PRO-TAB-VLM-ORCHESTRATION-SPEC.md`; publishable plan §P3 |
| **IMP-145** | Gameplay copy/debug hygiene and timer semantics. | 145.1 Remove internal phase labels from player-facing HUD across all build types (no debug/release fork). 145.2 Apply cosmetic timer label per `GAME-ENGINE` §7.3. 145.3 Keep Street View panel copy as “Location assist (text)” when no pano viewer exists. | IMP-083, IMP-144 | Gameplay has no internal engine/debug strings and no competitive timer implication. | `docs/GAME-ENGINE.md` §7.3; `rules/15`; publishable plan §2.5/§P4 |
| **IMP-146** | Share UX parity for results (all platforms). | Implement/normalize share flow with loading/success/error overlays, dismissible states, and non-blocking navigation; keep `navigator.share` + clipboard fallback on Web. | IMP-084 | Share UX behavior is consistent enough across platforms and does not block shell navigation. | publishable plan §P4; `docs/SOCIAL-AND-COMPETITION.md` |
| **IMP-147** | Permissive manifest skew handling + local-first sync hardening. | 147.1 On mismatch/unplayable server snapshot, overlay shipped round truth so app access remains available. 147.2 Avoid blocking gameplay solely for `content_version` skew; keep messaging lightweight/clear. 147.3 Persist guesses locally first and sync when possible with retry/idempotent replay. 147.4 Keep merge/outbox tests aligned with permissive behavior and retry semantics. | IMP-080, IMP-081 | Core SCAN/gameplay remains usable without forced updates; guesses are durable and eventually synced when online (outbox flush on launch/route entry). | publishable plan §2.4/§P5; `docs/PUBLISHABLE-UI-EXIT-CRITERIA.md` |
| **IMP-148** | Visual regression guardrail for publishable UI. | **Deferred** in current plan iteration (AD-5): no screenshot baseline pipeline or UI CI gate additions in this wave. | — | Deferred; revisit only if product/plan re-enables UI visual-regression gates. | publishable plan §P7; `rules/11` |
| **IMP-149** | Release sign-off workflow for player-facing builds. | Run exit checklist + store/demo asset checklist (`1024` icon, feature graphic, privacy policy URL); document deviations and waivers. | IMP-145, IMP-146, IMP-147, IMP-051 | `docs/PUBLISHABLE-UI-EXIT-CRITERIA.md` fully signed off for release branch. | publishable plan §P8; `docs/PUBLISHABLE-UI-EXIT-CRITERIA.md`; `rules/15` |
...
IMP-000 / IMP-001
  ...
  Publishable UI branch:
  IMP-141/142 → IMP-143/144 → IMP-145/146 → IMP-147 → IMP-149
            ∥
       IMP-051 (BGM asset completion)
  IMP-148 deferred (AD-5)
...
| 1.2 | 2026-04-21 | **§0.1:** cross-ref gap **v1.6** / claims **v1.3**; **IMP-081** / **W5 IMP-081** = shipped stills **landed** in **git**; **IMP-060+** / **IMP-110** §13.4 partial (batch rank merge + **`model_pins.json`**) documented. |
| 1.3 | 2026-04-21 | Added **W11** publishable UI wave (**IMP-140–149**) with deps/acceptance/refs; updated **§0.1** status table and dependency diagram with publishable branch + **IMP-051** coupling. |
| 1.4 | 2026-04-21 | W11 aligned with plan v0.4 AD decisions: **IMP-084** Web share status corrected; **IMP-145/146/147** rewritten for no debug/release fork, share overlay lifecycle, and permissive offline-first skew handling + local-first sync; **IMP-148 deferred** and dependency graph updated. |
| 1.5 | 2026-04-21 | **§0.1** cross-refs gap **v1.7** / claims **v1.4**; **IMP-020** / **IMP-072** rows: photo gallery removed; SCAN catalog helper **`refreshScanHubCatalog`**. |
| 1.6 | 2026-04-27 | W11 status refresh: **IMP-147** wording aligned to landed permissive merge + local-first outbox retry behavior; **IMP-148** deferred wording retained; **IMP-140–149** band moved from planned to active/in-progress. |
