...
def fetch_vlm_images(*, client: NutonicServerClient, job: ProJobStatusOut) -> list[FetchedImage]:
    ...
    for ref in payload.vlm_image_set:
        b = _fetch_ref_bytes(client=client, job_id=job.job_id, ref=ref)
        out.append(
            FetchedImage(
                role=ref.role,
                mime=ref.mime,
...
def _fetch_ref_bytes(*, client: NutonicServerClient, job_id: str, ref: ProVlmImageRef) -> bytes:
    url = ref.url or ref.inline_ref
    if url:
        return client.get_bytes_by_url(url)
    if ref.artifact_id:
        return client.get_artifact(job_id=job_id, artifact_id=ref.artifact_id)
