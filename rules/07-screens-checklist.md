...
| 9 | **PRO** (VLM tools, non-game) | **`refs/VLMExample/`** (port target) | `music_pro` | **Compose port** of the VLM example app: minimal shell, **not** the default geo-guess loop (`rules/01-navigation-architecture.md`, `rules/06-server-vlm-tim-and-on-device-ml.md`); **header music toggle** |
