...
def main() -> int:
    ...
    # 1) Copy everything except dirs we rebuild (images, mapbox_stills, overlays, analysis_images, metadata)
    ...
    if readme_dst.is_file():
        note = (
            ...
            f"``images/``, ``mapbox_stills/``, ``overlays/``, ``analysis_images/``, and ``metadata/`` use at most **{max_per}** "
