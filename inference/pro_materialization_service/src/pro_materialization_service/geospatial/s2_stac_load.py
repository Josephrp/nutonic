"""Sentinel-2 L2A patch loading (STAC + rasterio), TerraMind band order.
...
# TerraMind S2L2A channel order → Earth Search ``sentinel-2-l2a`` asset ids.
...
def apply_reflectance_scale(stack: np.ndarray, s2_cfg: Mapping[str, Any] | None = None) -> np.ndarray:
    ...
    cfg = s2_cfg or {}
    max_ref = float(cfg.get("reflectance_saturation_hint", 1.5))
    scale_to_dn = float(cfg.get("scale_to_dn_if_under", 10_000.0))
