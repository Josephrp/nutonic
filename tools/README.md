...
- **Full hydration (Jobs only — recommended):** [`run_full_hydration.py`](run_full_hydration.py) submits GPU `sv-lfm`, GPU TerraMind `tim`, then CPU `llm-sidecars`, waits, and downloads outputs ([`run_hf_hydration_full.py`](run_hf_hydration_full.py)). **Does not load LFM weights locally.** Optional Street View tuning: `--shuffle-seed`, `--pano-sampling-mode`, `--pano-jitter-seed`, `--pano-area-radius-m`, … (see [`hf_jobs/pano_batch_env.py`](hf_jobs/pano_batch_env.py) and [`hf_jobs/README.md`](hf_jobs/README.md)).
...
**Normative spec:** [`docs/scripts/SPEC-batch-streetview-hints.md`](../docs/scripts/SPEC-batch-streetview-hints.md). CLI flags `--pano-sampling-mode`, `--shuffle-seed`, `--pano-area-radius-m`, etc., match the Hugging Face Job env mapping in [`hf_jobs/pano_batch_env.py`](hf_jobs/pano_batch_env.py).
