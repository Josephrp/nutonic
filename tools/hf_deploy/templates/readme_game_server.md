...
```bash
python tools/hf_deploy/deploy_space.py --service game_server --repo-id NuTonic/nutonic-game-server
```

The workflow first runs `pytest server/tests -q`, then mirrors the staged Docker Space tree and syncs runtime settings from `tools/hf_deploy/profiles/game_server.yaml`.
...
The CI profile currently sets ranked, community writes, PRO jobs, and manifest truth to `false`. Enable them deliberately in Space settings or the profile when the connected databases/workers are ready.
