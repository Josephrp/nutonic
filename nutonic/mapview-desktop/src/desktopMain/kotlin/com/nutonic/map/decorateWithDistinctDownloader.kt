...
private sealed interface Message<K, T> {
    class StartDownload<K, T>(
        val key: K,
        val deferred: CompletableDeferred<T>,
    ) : Message<K, T>

    class DownloadComplete<K, T>(
        val key: K,
        val result: T,
    ) : Message<K, T>

    class DownloadFail<K, T>(
        val key: K,
...
fun <K, T> ContentRepository<K, T>.decorateWithDistinctDownloader(scope: CoroutineScope): ContentRepository<K, T> {
    val origin = this
    val actor =
        scope.actor<Message<K, T>> {
            ...
            while (true) {
                when (val message = receive()) {
                    is Message.StartDownload<K, T> -> {
                        val requestsWithSameKey =
                            mapKeyToRequests.getOrPut(message.key) {
