...
def drop_time_dim(value, dim: int = 0):
    ...
    if (isinstance(value, np.ndarray) or isinstance(value, torch.Tensor)):
        # Only squeeze when the requested dim actually exists
        if value.ndim > dim and value.shape[dim] == 1:
            return value.squeeze(dim)
        return value

    elif isinstance(value, dict):
        for k, v in value.items():
            # Recursive call so nested structures are also handled
            value[k] = drop_time_dim(v, dim=dim)
        return value
...
class MultimodalTransforms:
    ...
    def __call__(self, data: dict):
        # Split the incoming dict into image‑only and non‑image parts
        non_image_data = {
            k: v for k, v in data.items() if k in self.non_image_modalities
        }

        # pick the first modality that is not a non‑image key, it becomes the "primary" image
        image_modality = next(
            k for k in data.keys() if k not in self.non_image_modalities
        )
        primary_image = data.pop(image_modality)

        # Build the dict that Albumentations expects (image + any additional image targets)
        alb_input = {"image": primary_image}
        for k, v in data.items():
            if k not in self.non_image_modalities:
                alb_input[k] = v

        # Run the Albumentations pipeline
        alb_output = self.transforms(**alb_input)

        # put the primary image back under its original name
        result = {**non_image_data}
        result[image_modality] = alb_output.pop("image")

        # add the remaining (still‑image) modalities
        for k, v in alb_output.items():
            result[k] = v

        # Apply optional transforms to non‑image modalities
        for modality in self.non_image_modalities:
            if modality in result:
                result[modality] = self.non_image_transforms(result[modality])
