...
| **IMP-140–149** | **Planned (new band):** publishable UI / vendored parity / release readiness per **`plans/2026-04-21-publishable-ui-stitch-parity-and-ship-criteria.md`**. **Current status:** strategy + docs/rules are landed; implementation items remain open (SCAN convergence, design components, checklist completion, **permissive offline-first** manifest + guess sync, release sign-off). **IMP-148 (visual regression CI) is deferred by plan AD-5.** |
...
| **IMP-147** | Permissive manifest skew handling + local-first sync hardening. | 147.1 On mismatch/unplayable server snapshot, overlay shipped round truth so app access remains available. 147.2 Avoid blocking gameplay solely for `content_version` skew; keep messaging lightweight/clear. 147.3 Persist guesses locally and sync when possible with retry/idempotent replay. 147.4 Update merge tests for permissive behavior. | IMP-080, IMP-081 | Core SCAN/gameplay remains usable without forced updates; guesses are durable and eventually synced when online. | publishable plan §2.4/§P5; `docs/PUBLISHABLE-UI-EXIT-CRITERIA.md` |
