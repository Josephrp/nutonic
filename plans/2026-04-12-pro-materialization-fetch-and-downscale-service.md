...
| **Game server role** | **Thin orchestrator** only: HMAC/mTLS **`POST`** to this service → receives **`MaterializationResult`** → forwards tensor handles / NPZ to **`TERRAMIND_*_URL`** → merges **`tim_modality_outputs`** (all **`tim_modalities`**) into **`ProVisionBundle`**; **`AiGuessStore`** **`Coordinates` → ai_lat/ai_lon** only for **`map_id`** / registered cache rows (`plans/2026-04-07-game-server-thin-orchestrator.md` §1.6, `docs/PRO-TAB-VLM-ORCHESTRATION-SPEC.md` §5). |
