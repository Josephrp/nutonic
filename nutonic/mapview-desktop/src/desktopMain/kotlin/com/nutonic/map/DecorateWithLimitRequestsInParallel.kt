...
import com.nutonic.map.collection.ImmutableCollection
import com.nutonic.map.collection.createStack
...
fun <K, T> ContentRepository<K, T>.decorateWithLimitRequestsInParallel(
    ...
    val store =
        scope.createStoreWithSideEffect(
            init = State(),
...
private sealed interface Intent<K, T> {
    ...
    class NewElement<K, T>(
        val wait: ElementWait<K, T>,
    ) : Intent<K, T>
...
private sealed interface NetworkSideEffect<K, T> {
    class Load<K, T>(
        val waitElements: List<ElementWait<K, T>>,
    ) : NetworkSideEffect<K, T>
