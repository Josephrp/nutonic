...
- **`vlm_contract_id`**: **`nutonic.pro.vlm.v1_512`** → roles **`mapbox_rgb`** only. **`nutonic.pro.vlm.v1_512_fc_scl`** (plan §6.4) adds **`sentinel_fc`** (SWIR2/NIR/red false-color PNG) + **`cloud_mask_thumb`** (SCL → semi-transparent RGBA PNG); requires **`TERRAMIND_SPECTRAL`** or **`FULL_STAC`** and optional **`[s2]`** deps.
...
- **`MAPBOX_ACCESS_TOKEN`** required for all materialize modes today.

## Environment

| Variable | Purpose |
|----------|---------|
| **`MAPBOX_ACCESS_TOKEN`** | Mapbox Static Images token. |
