...
``mapbox_stills/``, optionally ``overlays/``). A full run exceeds that limit; commits fail with
HTTP **400** mentioning ``Offending directories: /images/``.

This script copies the dataset to a new root and:

* Moves each flat ``images/*.png``, ``mapbox_stills/*.png``, ``overlays/*.png`` into
  subfolders ``<dir>/s00000/``, ``<dir>/s00001/``, … with **at most** ``--max-files-per-dir``
  files per shard (default **8000**, safely under 10k).
* Re-writes **relative path strings** inside ``data/*.jsonl`` (and ``metadata/*.json``) so
  they match the new layout (e.g. ``images/poi_000001_t0000.png`` →
  ``images/s00003/poi_000001_t0000.png``).
...
# Top-level dataset folders that may contain a huge flat set of PNGs.
MEDIA_DIRS = ("images", "mapbox_stills", "overlays")
...
def _list_flat_media_files(media_root: Path) -> list[Path]:
    """Direct image files only (ignore subdirectories — those are separate layout modes)."""
    if not media_root.is_dir():
        return []
    out: list[Path] = []
    for child in media_root.iterdir():
        if child.is_file() and child.suffix.lower() in _IMAGE_SUFFIX:
            out.append(child)
...
def _materialize_media_tree(
    ...
    *,
    ...
    """Copy/link sharded media; return path remap old_posix -> new_posix."""
    remap: dict[str, str] = {}
...
def main() -> int:
    ...
    # 1) Copy everything except media dirs we rebuild from flat files
    skip = set(MEDIA_DIRS)
    for media in MEDIA_DIRS:
    ...
    # 2) Materialize sharded images / mapbox_stills / overlays
    remap = _materialize_media_tree(src_root, dst_root, max_per=max_per, link=args.link)
    ...
    # 4) Rewrite metadata JSON (usually no image paths; cheap to scan)
    meta_src = src_root / "metadata"
    if meta_src.is_dir():
        meta_dst = dst_root / "metadata"
        for js in meta_src.rglob("*.json"):
            rel = js.relative_to(meta_src)
            _rewrite_json_file(js, meta_dst / rel, remap)
    ...
    if readme_dst.is_file():
        note = (
            ...
            f"``images/``, ``mapbox_stills/``, and ``overlays/`` contain at most **{max_per}** "
            "files per subdirectory (Hub git limit: 10k files per directory). "
            "JSONL image paths include an intermediate ``sNNNNN/`` shard directory.\n"
        )
